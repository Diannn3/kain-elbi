...
### Freshness is evaluated at review time

The freshness stored on a claim records how old the evidence was when it was imported. The review queue recomputes freshness from the observation timestamps every time it is built, so old evidence cannot remain "fresh" forever. Queue IDs include the current evidence state; when evidence crosses a freshness boundary, old review sheets become stale and are rejected.
...
2. Export either from the CLI:
   ...
   or, for signed-in Places editors/owners, use **Export review CSV** in `/places-ops`. Viewer-role staff can inspect the reduced queue but cannot export the decision CSV. The endpoint returns the complete sanitized server-only queue and uses `private, no-store`; the hydrated dashboard receives only a bounded reduced snapshot, and raw observations/claims are never copied to `public/data`. Spreadsheet-formula-like cells are neutralized on export and unwrapped only by the decision importer.
...
`place_overrides.json` gives reviewed facts a durable, auditable location while preserving OSM/Overture snapshots and stable IDs as the underlying source graph. The research claim name is `operational_status`, but the generated canonical catalog field remains `operating_status`; the override layer performs that mapping explicitly so the public app sees one status field, not two.
