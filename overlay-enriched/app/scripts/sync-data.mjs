...
await syncRequired('place_enrichment.json', (value) => {
	if (!value || typeof value !== 'object' || value.version !== 1 || !value.places || typeof value.places !== 'object' || Array.isArray(value.places)) {
		throw new Error('place_enrichment.json must use version 1 and contain a places object');
	}
});
