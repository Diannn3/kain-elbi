...
const EVENT_TYPES = new Set(['visit_reported', 'accuracy_confirmed']);
...
Deno.serve(async (req) => {
	...
	if (!EVENT_TYPES.has(eventType)) return json(req, { error: 'invalid_event_type' }, 400);
	...
	try {
		const day = manilaDay();
		const { dedupeToken, dailyInstallToken } = await communityTokens({
			eventType,
			installId,
			placeId,
			day,
		...
		const { data, error } = await admin.rpc('record_uppetite_community_event', {
		...
		if (error) {
			...
			console.error('community-report rpc error', error);
