...
def _empty_report(input_error: str) -> dict:
    ...
    return {
        ...
        'snap_records': 0,
        'snap_status': {},
        'snap_thresholds_m': None,
        'max_routed_place_snap_m': None,
        'max_supported_anchor_snap_m': None,
        ...
        'route_coverage': {'status': 'not_applicable', 'release_ready': False, 'errors': [input_error]},
...
def build_report() -> dict:
    if not PLACES_FILE.exists():
        return _empty_report(f'places.json is missing: {PLACES_FILE}')
    try:
        places = json.loads(PLACES_FILE.read_text(encoding='utf-8'))
    except (OSError, json.JSONDecodeError) as exc:
        return _empty_report(f'places.json is unreadable or invalid JSON: {exc}')
    if not isinstance(places, list):
        return _empty_report('places.json must contain an array')
    if ROUTE_MATRIX_FILE.exists():
        try:
            route = json.loads(ROUTE_MATRIX_FILE.read_text(encoding='utf-8'))
...
def release_failures(report: dict) -> list[str]:
    failures: list[str] = list(report.get('input_errors') or [])
