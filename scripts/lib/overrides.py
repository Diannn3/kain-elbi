...
PERMANENTLY_CLOSED_STATUSES = {"permanently_closed", "permanently closed"}
    ...
    for place_id, entry in normalized["places"].items():
        ...
        for field, override in entry["fields"].items():
            ...
            if field == "coordinates":
                ...
                canonical_field = "operating_status" if field == "operational_status" else field
                place[canonical_field] = value
                if field == "operational_status":
                    # Never emit the research-layer spelling into generated places.json.
                    place.pop("operational_status", None)
            fields_applied += 1
        ...
        operating = str(place.get("operating_status") or "").casefold().replace(" ", "_")
        if "operational_status" in entry["fields"] and operating in PERMANENTLY_CLOSED_STATUSES:
