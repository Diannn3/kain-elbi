...
from lib.research import field_policy, load_jsonl, parse_datetime
...
def _recent_accepted_evidence(decisions_file: Path, now: datetime) -> dict[str, set[str]]:
    """Return evidence-only fields that were human-reviewed within their field TTL.

    Evidence-only facts (for example official social URLs) are intentionally not
    copied into the public place schema yet. Without this memory, every research
    batch would ask operators to rediscover the same accepted evidence forever.
    """
    recent: dict[str, set[str]] = {}
    for row in load_jsonl(decisions_file):
        if str(row.get("decision") or "").casefold() != "accept_evidence":
            continue
        ...
        try:
            ...
            ttl_days = field_policy(field).ttl_days
        except (ValueError, TypeError):
            continue
        age_days = max(0, int((now - reviewed_at).total_seconds() // 86_400))
        if age_days <= ttl_days:
            recent.setdefault(place_id, set()).add(field)
    return recent
...
def _desired_claims(
    ...
    now: datetime,
    *,
    recently_accepted_evidence: set[str] | None = None,
) -> tuple[list[str], list[str], int]:
    desired: list[str] = []
    reasons: list[str] = []
    priority = 0
    recently_accepted_evidence = recently_accepted_evidence or set()
    ...
    for evidence_field in ("facebook_url", "instagram_url"):
        if evidence_field not in recently_accepted_evidence:
            desired.append(evidence_field)
...
def build_research_batch(
    *,
    ...
    if not isinstance(snaps, dict):
        snaps = {}
    accepted_evidence = _recent_accepted_evidence(decisions_file, now)
    ...
    for place in places:
        ...
        desired, reasons, priority = _desired_claims(
            place, extra, now, recently_accepted_evidence=accepted_evidence.get(place_id, set())
        )
