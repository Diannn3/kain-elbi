...
MEAL_TAGS = {"rice-meal", "snack", "coffee", "dessert", "heavy-meal", "quick-meal", "bakery", "drinks"}
...
def _is_future_date(value: Any, now: datetime) -> bool:
    return _valid_date(value) and date.fromisoformat(value) > now.date()
    ...
    *,
    ...
    for place_id, raw in entries.items():
        ...
        for field in ("addedAt", "lastReviewedAt"):
            ...
            elif _is_future_date(raw.get(field), now):
                errors.append(f"enrichment {place_id} {field} cannot be in the future")
        ...
        if price is not None:
            if not isinstance(price, dict):
                ...
                elif _is_future_date(verified, now):
                    errors.append(f"enrichment {place_id} price verifiedAt cannot be in the future")
                else:
            ...
            if not isinstance(verification, dict):
                ...
                for field, record in verification.items():
                    ...
                    if not _valid_date(record.get("verifiedAt")) or record.get("source") not in VERIFICATION_SOURCES:
                        errors.append(f"enrichment {place_id} verification.{field} is invalid")
                    elif _is_future_date(record.get("verifiedAt"), now):
                        errors.append(f"enrichment {place_id} verification.{field}.verifiedAt cannot be in the future")
        ...
        if shop is not None:
            ...
            elif _is_future_date(shop.get("verifiedAt"), now):
                errors.append(f"enrichment {place_id} shopVerification.verifiedAt cannot be in the future")
            else:
                verified_shops += 1

        meal_tags = raw.get("mealTags")
        if meal_tags is not None:
            if not isinstance(meal_tags, list) or any(tag not in MEAL_TAGS for tag in meal_tags) or len(set(meal_tags)) != len(meal_tags):
                errors.append(f"enrichment {place_id} mealTags are invalid")

        dishes = raw.get("dishes")
        if dishes is not None:
            if not isinstance(dishes, list):
                errors.append(f"enrichment {place_id} dishes must be an array")
            else:
                seen_dishes: set[str] = set()
                for index, dish in enumerate(dishes):
                    if not isinstance(dish, dict) or not isinstance(dish.get("name"), str) or not dish.get("name", "").strip():
                        errors.append(f"enrichment {place_id} dish {index} needs a name")
                        continue
                    dish_key = dish["name"].strip().casefold()
                    if dish_key in seen_dishes:
                        errors.append(f"enrichment {place_id} contains duplicate dish {dish['name']!r}")
                    seen_dishes.add(dish_key)
                    price_value = dish.get("pricePhp")
                    if price_value is not None and (not isinstance(price_value, int) or isinstance(price_value, bool) or not 1 <= price_value <= 10_000):
                        errors.append(f"enrichment {place_id} dish {index} pricePhp is invalid")
                    tags = dish.get("tags")
                    if tags is not None and (not isinstance(tags, list) or any(not isinstance(tag, str) or not tag.strip() for tag in tags)):
                        errors.append(f"enrichment {place_id} dish {index} tags are invalid")
                    dish_verified = dish.get("verifiedAt")
                    if dish_verified is not None:
                        if not _valid_date(dish_verified):
                            errors.append(f"enrichment {place_id} dish {index} verifiedAt must use YYYY-MM-DD")
                        elif _is_future_date(dish_verified, now):
                            errors.append(f"enrichment {place_id} dish {index} verifiedAt cannot be in the future")
