  ...
  <section class="research" aria-labelledby="research-title">
    <div class="heading research-heading">
      ...
      <div class="research-actions">
        ...
        {#if editable}<a href="/api/ops/research-export">Export review CSV</a>{/if}
      </div>
    </div>
    <p class="research-note">Agent research never edits the canonical catalog directly. {editable ? 'Editors can export this queue, review it in Sheets, then apply the reviewed CSV through the research decision tool.' : 'You have read-only access to the research queue.'} Stale queue IDs are rejected automatically.</p>
                  ...
                  {#if proposal.sourceUrls.length}
                    <nav aria-label={`Evidence sources for ${item.placeName}`}>
                      {#each proposal.sourceUrls.slice(0, 4) as url, sourceIndex}
                        <a href={url} target="_blank" rel="noopener noreferrer">Source {sourceIndex + 1}</a>
                      {/each}
                    </nav>
                  {/if}
                </div>
      ...
      {#each filtered.slice(0, 100) as task, index (task.placeId)}
        <article>
          ...
          <div class="task-actions">
            <a href={`/place/${encodeURIComponent(task.placeId)}`} target="_blank" rel="noopener noreferrer">Open</a>
            <a href={`/contribute?place=${encodeURIComponent(task.placeId)}#suggest-edit`} target="_blank" rel="noopener noreferrer">Verify</a>
          </div>
        </article>
      {:else}
