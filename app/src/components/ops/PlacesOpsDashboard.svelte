  ...
  <section class="queue" aria-labelledby="queue-title">
    ...
    <div class="tasks">
      {#each filtered.slice(0, 100) as task, index (task.placeId)}
        <article><span class="rank">{String(index + 1).padStart(2, '0')}</span><div><strong>{task.placeName}</strong><p>{task.reasons.join(' · ')}</p></div><b>{task.priority}</b><div class="task-actions"><a href={`/place/${encodeURIComponent(task.placeId)}`} target="_blank">Open</a><a href={`/contribute?place=${encodeURIComponent(task.placeId)}#suggest-edit`} target="_blank">Verify</a></div></article>
