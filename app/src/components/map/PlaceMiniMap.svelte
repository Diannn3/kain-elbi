...
<style>
	.location-map { position: relative; min-height: 15rem; overflow: hidden; border-radius: var(--radius-lg); background: var(--surface-subtle); }
	.map-canvas { position: absolute; inset: 0; }
	.map-fallback { position: absolute; inset: 0; display: grid; place-content: center; justify-items: center; gap: var(--space-3); background: radial-gradient(circle at 50% 45%, hsl(44 96% 49% / 0.2), transparent 7rem), var(--surface-subtle); color: var(--text-secondary); text-align: center; }
	.map-fallback p { margin: 0; font-size: 0.75rem; }
	.pin { position: relative; display: grid; place-items: center; width: 3rem; height: 3rem; border-radius: 50% 50% 50% 0; background: var(--forest); rotate: -45deg; box-shadow: 0 0.7rem 1.4rem hsl(154 40% 10% / 0.2); }
	.pin i { width: 0.9rem; height: 0.9rem; border-radius: 50%; background: var(--sun); }
	:global(.place-location-marker) { width: 2rem; height: 2rem; border: 0.35rem solid white; border-radius: 50%; background: var(--sun); box-shadow: 0 0.45rem 1.2rem hsl(154 55% 8% / 0.3); }
