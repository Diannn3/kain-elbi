...
export function buildOpsDashboard(places: Place[], now = new Date()): OpsDashboard {
  const activePlaces = places.filter((place) => place.recordStatus === 'candidate');
  const tasks: OpsTask[] = [];
  let stale = 0, missingHours = 0, missingPrice = 0, missingMealTags = 0, missingDishes = 0, verifiedShops = 0;
  ...
  for (const place of activePlaces) {
    ...
    if (reasons.length) tasks.push({ placeId: place.id, placeName: place.name, priority, reasons });
  }

  tasks.sort((a, b) => b.priority - a.priority || a.placeName.localeCompare(b.placeName));
  return {
    generatedAt: now.toISOString(),
    totals: { places: activePlaces.length, stale, missingHours, missingPrice, missingMealTags, missingDishes, verifiedShops },
