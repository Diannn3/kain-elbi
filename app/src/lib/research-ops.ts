  ...
  const items = (Array.isArray(root.items) ? root.items : []).flatMap((raw) => {
    ...
    const proposals = (Array.isArray(item.proposals) ? item.proposals : []).flatMap((rawProposal) => {
      ...
      return [{
        ...
        sourceUrls: strings(proposal.source_urls).flatMap((url) => {
...
function spreadsheetSafe(value: string): string {
  // CSV quoting does not neutralize spreadsheet formulas. Prefix formula-like
  // cells with a tab; the Python decision importer removes exactly this guard.
  return value && FORMULA_PREFIXES.has(value[0]) ? `\t${value}` : value;
}

function csvCell(value: unknown): string {
  if (value === null || value === undefined) return '';
  const raw = typeof value === 'string' ? value : JSON.stringify(value);
  return `"${spreadsheetSafe(String(raw)).replaceAll('"', '""')}"`;
}
